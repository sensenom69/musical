    ...
    */
    private static function getInput($camp){
        if(strpos($camp,"id_")!==FALSE){
            return '<select id="'.$camp.'" name="'.$camp.'" ><option value="0">-</option><!--$ INI '.$camp.' --><option value="##'.$camp.'##" ##selected##>##nombre##</option><!--$ FIN '.$camp.' --></select>';
            
        }elseif(strpos($camp,"descripcio")!==FALSE OR strpos($camp,"message")!==FALSE ){
            return '<textarea ##editable## class="textarea_llarg" ##readonly## id="'.$camp.'" name="'.$camp.'">##'.$camp.'##</textarea>';
            ...
            return '<input id="'.$camp.'" name="'.$camp.'" value="##'.$camp.'##" type="text" ##editable##/>';   
        }
    }
